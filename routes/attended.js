...
function canEditAttended(req, row) {
  ...
  if (['hod', 'iqac_dept'].includes(role) && req.user.department && req.user.department === row.department) return true;
  ...
  try {
    await pg(
      `INSERT INTO iqac_remark_threads
       ...
       VALUES (?,?,?,?,?,?,?,?)`,
      ['attended', recordId, action || '', message || '', roleOf(req), toRole || '', req.user.email || req.user.empid || '', req.user.empid || '']
    );
  } catch (err) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!canEditAttended(req, row)) return res.status(403).json({ error: 'Insufficient permissions' });
...
router.get('/:id/thread', async (req, res) => {
  try {
    const record = await pg('SELECT * FROM events_attended WHERE id = ?', [req.params.id]);
    if (!record.rows.length) return res.status(404).json({ error: 'Record not found' });
    if (!canEditAttended(req, record.rows[0])) return res.status(403).json({ error: 'Insufficient permissions' });

    const result = await pg(
      `SELECT * FROM iqac_remark_threads
       WHERE record_type = 'attended' AND record_id = ?
  ...
  try {
    ...
    const result = await pg(`
      INSERT INTO events_attended
        (faculty_name, department, event_name, event_type, event_date, academic_year, proof_file, submitted_by, status)
      VALUES (?,?,?,?,?,?,?,?,?)
      RETURNING id`,
      [faculty_name, department, event_name, event_type||'', event_date||null,
       academic_year||'', proof, req.user.empid, 'Submitted']
    );

    await addThread(result.rows[0].id, req, 'Submitted', 'Events-attended record submitted.', 'hod');
...
router.put('/:id', upload.single('proof'), async (req, res) => {
  try {
    const oldRes = await pg('SELECT * FROM events_attended WHERE id = ?', [req.params.id]);
    if (!oldRes.rows.length) return res.status(404).json({ error: 'Record not found' });

    const old = oldRes.rows[0];
    if (!canEditAttended(req, old)) return res.status(403).json({ error: 'Insufficient permissions' });

    const { faculty_name, department, event_name, event_type, event_date, academic_year, faculty_reply } = req.body;
    ...
    await pg(
      `UPDATE events_attended SET
        faculty_name = ?, department = ?, event_name = ?, event_type = ?,
        event_date = ?, academic_year = ?, proof_file = ?, status = ?
       WHERE id = ?`,
      [
    ...
    await addThread(old.id, req, 'Edited/Reuploaded', faculty_reply || 'Events-attended data/proof updated and resubmitted.', 'reviewer');
...
router.post('/:id/reply', async (req, res) => {
  try {
    ...
    const isOwner = String(row.submitted_by || '') === String(req.user.empid || '');
    const status = isOwner ? 'Faculty Replied' : 'Remark Given';

    await pg(`UPDATE events_attended SET status = ? WHERE id = ?`, [status, row.id]);
    await addThread(row.id, req, isOwner ? 'Faculty Reply' : 'Reviewer Remark', message, isOwner ? 'reviewer' : 'faculty');

    res.json({ message: 'Reply saved successfully', status });
  } catch (err) {
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await pg('SELECT * FROM events_attended WHERE id = ?', [req.params.id]);
    if (!result.rows.length) return res.status(404).json({ error: 'Record not found' });
    if (!canEditAttended(req, result.rows[0])) return res.status(403).json({ error: 'Insufficient permissions' });

    await deleteFile(result.rows[0].proof_file);
    await pg('DELETE FROM events_attended WHERE id = ?', [req.params.id]);
    await pg(`DELETE FROM iqac_remark_threads WHERE record_type = 'attended' AND record_id = ?`, [req.params.id]);
